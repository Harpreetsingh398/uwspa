...
# Configuration
st.set_page_config(layout="wide", page_title="Precision Wind Analytics", page_icon="🌬️")

# Custom CSS
st.markdown("""
<style>
    .main {background-color: #0E1117; color: white;}
    .metric-card {border-radius: 10px; padding: 15px; background-color: #1E1E1E; box-shadow: 0 4px 6px rgba(0,0,0,0.1);}
    .st-bb {background-color: #262730;}
    .st-at {background-color: #262730;}
    .stTextInput input, .stSelectbox select, .stSlider div {background-color: #262730; color: white;}
</style>
""", unsafe_allow_html=True)
...
# Turbine Model
class WindTurbine:
    def __init__(self, name, cut_in, rated, cut_out, max_power):
        self.name = name
        self.cut_in = cut_in
        self.rated = rated
        self.cut_out = cut_out
...
# Turbine Database
TURBINES = {
    "Vestas V80-2.0MW": WindTurbine("Vestas V80-2.0MW", 4, 15, 25, 2000),
    "GE 1.5sle": WindTurbine("GE 1.5sle", 3.5, 14, 25, 1500),
    "Suzlon S88-2.1MW": WindTurbine("Suzlon S88-2.1MW", 3, 12, 25, 2100)
}
...
def train_wind_model(df):
    ...
    # Add lag features
    for lag in [1, 3, 6]:
        df[f'wind_lag{lag}'] = df['Wind Speed (m/s)'].shift(lag)
    
    # Add rolling stats
    df['rolling_mean'] = df['Wind Speed (m/s)'].rolling(6).mean()
    df['rolling_std'] = df['Wind Speed (m/s)'].rolling(6).std()
    
    # Drop NA
    df = df.dropna()
    
    # Prepare data
    X = df.drop(['Time', 'Wind Speed (m/s)'], axis=1)
    y = df['Wind Speed (m/s)']
    ...
    return model, X.columns.tolist(), np.mean(r2_scores)

def predict_wind(model, features, last_data, hours_ahead):
    pred_data = []
    for i in range(1, hours_ahead + 1):
        time = last_data['Time'] + timedelta(hours=i)
        ...
        row = {
            ...
            'wind_lag1': last_data['Wind Speed (m/s)'],
            'wind_lag3': last_data.get('wind_lag2', last_data['Wind Speed (m/s)']),
            'wind_lag6': last_data.get('wind_lag5', last_data['Wind Speed (m/s)']),
            'rolling_mean': last_data.get('rolling_mean', last_data['Wind Speed (m/s)']),
            'rolling_std': last_data.get('rolling_std', 0)
        }
        pred_data.append(row)
    ...
    return [last_data['Time'] + timedelta(hours=i) for i in range(1, hours_ahead + 1)], predictions

# Main App
def main():
    st.title("🌬️ Precision Wind Energy Analytics")
    ...
    with st.expander("⚙️ Configuration", expanded=True):
        ...
        with col1:
            ...
            days = st.slider("📅 Forecast Days", 1, 5, 2)
        with col2:
            turbine_model = st.selectbox("🌀 Turbine Model", list(TURBINES.keys()))
            future_hours = st.slider("🔮 Predict Ahead (hours)", 6, 48, 24)
    ...
    if st.button("🚀 Analyze"):
        with st.spinner("Processing..."):
            ...
            st.success(f"📍 {display_name} (Lat: {lat:.4f}, Lon: {lon:.4f})")
            ...
            model, features, r2_score = train_wind_model(df.copy())
            
            # Make predictions
            last_data = df.iloc[-1].to_dict()
            pred_times, pred_speeds = predict_wind(model, features, last_data, future_hours)
            
            # Create prediction dataframe
            pred_df = pd.DataFrame({
                'Time': pred_times,
                'Predicted Wind Speed (m/s)': pred_speeds,
                'Predicted Power (kW)': turbine.power_output(pred_speeds)
            })
            
            # Key Metrics
            st.subheader("📊 Key Metrics")
            cols = st.columns(4)
            cols[0].metric("Avg Wind Speed", f"{df['Wind Speed (m/s)'].mean():.2f} m/s")
            cols[1].metric("Max Wind Speed", f"{df['Wind Speed (m/s)'].max():.2f} m/s")
            cols[2].metric("Power Density", f"{0.5 * 1.225 * (df['Wind Speed (m/s)']**3).mean():.1f} W/m²")
            cols[3].metric("Model R² Score", f"{r2_score:.2%}")
            
            # Main Tabs
            tab1, tab2, tab3 = st.tabs(["Wind Analysis", "Turbine Performance", "AI Forecast"])
            ...
            with tab1:
                ...
                with col2:
                    fig = px.scatter_polar(df, r="Wind Speed (m/s)", theta="Wind Direction",
            ...
            with tab3:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Avg Predicted Speed", 
                             f"{pred_df['Predicted Wind Speed (m/s)'].mean():.2f} m/s")
                with col2:
                    st.metric("Total Predicted Energy", 
                             f"{pred_df['Predicted Power (kW)'].sum()/1000:.2f} MWh")
                
                fig = px.line(pred_df, x="Time", y="Predicted Power (kW)",
