...
def generate_sample_data(days=7):
    now = datetime.now()
    times = [now - timedelta(hours=i) for i in range(24*days, 0, -1)]
    np.random.seed(42)
    
    base_wind = np.sin(np.linspace(0, days*2*np.pi, 24*days)) * 5 + 10
    noise = np.random.normal(0, 2, 24*days)
    
    df = pd.DataFrame({
        "Time": times,
        "Wind Speed (m/s)": np.clip(base_wind + noise, 0, 25),
        "Wind Direction": np.random.randint(0, 360, 24*days),
        "Temperature (°C)": np.sin(np.linspace(0, days*2*np.pi, 24*days)) * 10 + 20,
        "Humidity (%)": np.random.randint(30, 90, 24*days),
        "Pressure (hPa)": np.random.normal(1013, 5, 24*days)
    })
    return df
...
# Enhanced Wind Speed Prediction Model
def train_wind_speed_model(df, forecast_hours=24):
    # Feature engineering
    df['hour'] = df['Time'].dt.hour
    df['hour_sin'] = np.sin(2 * np.pi * df['hour']/24)
    df['hour_cos'] = np.cos(2 * np.pi * df['hour']/24)
    ...
    # Features and target
    features = ['hour_sin', 'hour_cos', 'day_of_week', 'day_of_year',
               'Temperature (°C)', 'Humidity (%)', 'Pressure (hPa)',
    ...
    future_times = [last_data['Time'] + timedelta(hours=i) for i in range(1, forecast_hours+1)]
    ...
    for i, time in enumerate(future_times):
        ...
        row = {
            ...
            'Temperature (°C)': last_data['Temperature (°C)'] * 0.98,  # Small decay
            'Humidity (%)': min(last_data['Humidity (%)'] * 1.02, 100),  # Small increase
            'Pressure (hPa)': last_data['Pressure (hPa)'] * 0.999,
            'temp_humidity': last_data['Temperature (°C)'] * last_data['Humidity (%)'],
            'pressure_temp': last_data['Pressure (hPa)'] / last_data['Temperature (°C)'],
            'wind_speed_lag1': last_data['Wind Speed (m/s)'],
            'wind_speed_lag2': df.iloc[-2]['Wind Speed (m/s)'] if len(df) > 1 else last_data['Wind Speed (m/s)'],
            'wind_speed_lag3': df.iloc[-3]['Wind Speed (m/s)'] if len(df) > 2 else last_data['Wind Speed (m/s)']
        }
    ...
    return model, features, test_r2, test_mae, future_times, future_pred
...
def main():
    ...
    with st.expander("⚙️ Configuration Panel", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            historical_days = st.slider("Days of Historical Data", 1, 14, 7)
            turbine_model = st.selectbox("Turbine Model", list(TURBINES.keys()))
        
        with col2:
            forecast_hours = st.slider("Hours to Forecast", 6, 48, 24)
            show_metrics = st.checkbox("Show Model Metrics", True)
    
    # Load data
    df = generate_sample_data(historical_days)
    now = datetime.now()
    
    # Calculate air density and power
    df['Air Density (kg/m³)'] = calculate_air_density(
        df['Temperature (°C)'], 
        df['Humidity (%)'], 
        df['Pressure (hPa)']
    ...
    turbine = TURBINES[turbine_model]
    df['Power Output (kW)'] = turbine.power_output(df['Wind Speed (m/s)'])
    df['Energy Output (kWh)'] = df['Power Output (kW)']
    
    # Train model and make predictions
    model, features, test_r2, test_mae, future_times, future_pred = train_wind_speed_model(df, forecast_hours)
    
    # Create prediction dataframe
    pred_df = pd.DataFrame({
        'Time': future_times,
        'Predicted Wind Speed (m/s)': future_pred,
        'Lower Bound': future_pred * 0.95,
        'Upper Bound': future_pred * 1.05
    })
    
    # Combine historical and predicted data
    combined_df = pd.concat([
        df[['Time', 'Wind Speed (m/s)', 'Power Output (kW)', 'Energy Output (kWh)']],
        pred_df
    ], ignore_index=True)
    ...
    col1.metric("Average Wind Speed", f"{df['Wind Speed (m/s)'].mean():.2f} m/s")
    col2.metric("Max Wind Speed", f"{df['Wind Speed (m/s)'].max():.2f} m/s")
    col3.metric("Total Energy", f"{df['Energy Output (kWh)'].sum()/1000:.1f} MWh")
    
    # Main Visualization
    st.subheader("🌪️ Wind Speed Analysis & Prediction")
    ...
    fig.update_layout(
        title=f"Wind Speed Forecast - Next {forecast_hours} Hours",
        xaxis_title="Time",
        yaxis_title="Wind Speed (m/s)",
        template="plotly_dark",
        hovermode="x unified"
    ...
    if show_metrics:
        ...
        col1, col2 = st.columns(2)
        col1.metric("Model Accuracy (R²)", f"{test_r2:.2%}")
        col2.metric("Mean Absolute Error", f"{test_mae:.2f} m/s")
        
        st.markdown("""
        **Model Details**:
        - Algorithm: Random Forest Regressor (300 trees)
        - Features: Temporal patterns, weather conditions, and historical wind speeds
        - Training Data: Last {} days of historical data
        """.format(historical_days))
    
    # Turbine Performance
    st.subheader("🌀 Turbine Performance Forecast")
    
    # Calculate predicted power
    pred_power = turbine.power_output(pred_df['Predicted Wind Speed (m/s)'])
