...
#                 with st.spinner("🔍 Loading historical wind data..."):
#                     historical_data = get_weather_data(lat, lon, days=3)  # Get 3 days of historical data
...
#                     # Filter to only keep data before current time
#                     current_time = datetime.now()
#                     hist_df = hist_df[hist_df['Time'] < current_time]
...
#                 # Get additional historical data for validation
#                 with st.spinner("🔍 Analyzing historical wind patterns..."):
#                     validation_data = get_weather_data(lat, lon, days=5)
#                     val_df = pd.DataFrame({
#                         "Time": pd.to_datetime(validation_data['hourly']['time']),
#                         "Actual Wind Speed (m/s)": validation_data['hourly']['wind_speed_10m'],
#                         "Wind Direction": validation_data['hourly']['wind_direction_10m'],
#                         "Temperature (°C)": validation_data['hourly']['temperature_2m'],
#                         "Humidity (%)": validation_data['hourly']['relative_humidity_2m'],
#                         "Pressure (hPa)": validation_data['hourly']['surface_pressure']
#                     })
                    
#                     # Feature engineering
#                     val_df['hour'] = val_df['Time'].dt.hour
#                     val_df['hour_sin'] = np.sin(2 * np.pi * val_df['hour']/24)
#                     val_df['hour_cos'] = np.cos(2 * np.pi * val_df['hour']/24)
#                     val_df['day_of_week'] = val_df['Time'].dt.dayofweek
#                     val_df['day_of_year'] = val_df['Time'].dt.dayofyear
#                     val_df['month'] = val_df['Time'].dt.month
#                     val_df['wind_speed_lag1'] = val_df['Actual Wind Speed (m/s)'].shift(1)
#                     val_df['wind_speed_lag2'] = val_df['Actual Wind Speed (m/s)'].shift(2)
#                     val_df['wind_speed_lag3'] = val_df['Actual Wind Speed (m/s)'].shift(3)
#                     val_df = val_df.dropna()
                    
#                     # Train-test split (most recent 20% for testing)
#                     test_size = int(len(val_df) * 0.2)
#                     train_df = val_df.iloc[:-test_size]
#                     test_df = val_df.iloc[-test_size:]
    ...
    if st.button("🚀 Analyze Wind Data"):
        with st.spinner("Fetching wind data and performing analysis..."):
            ...
            with tab4:
                ...
                # Get historical data for past days
                with st.spinner("🔍 Loading historical wind data..."):
                    # Get more historical data (5 days) for better visualization
                ...
                # Predict future wind speeds starting from current time
                if not hist_df.empty:
                    # Get the most recent data point to start predictions from
                    last_data_point = hist_df.iloc[-1].to_dict()
                    ...
                    # Combine historical and forecasted data for continuous visualization
                    combined_df = pd.concat([
                        hist_df[['Time', 'Wind Speed (m/s)']].assign(Type='Historical'),
                        pred_df[['Time', 'Wind Speed (m/s)']].assign(Type='Forecast')
                    ])
                    
                    # Plot combined historical + forecast data with confidence band
                    fig = go.Figure()
                    
                    # Historical data - last 24 hours for better visualization
                    last_24h = hist_df[hist_df['Time'] >= (current_time - timedelta(hours=24))]
                    fig.add_trace(go.Scatter(
                        x=last_24h['Time'], 
                        y=last_24h['Wind Speed (m/s)'], 
                        name='Historical Data',
                    ...
                    fig.update_layout(
                        title=f"Continuous Wind Speed Forecast: Past 24 Hours + Next {future_hours} Hours",
                        xaxis_title="Time",
                        yaxis_title="Wind Speed (m/s)",
                        template="plotly_dark",
                        hovermode="x unified",
